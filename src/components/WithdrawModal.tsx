...
export const WithdrawModal: React.FC<WithdrawModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md font-sans overflow-y-auto">
      ...
      >
        {/* Modern Gradient Header — amber/orange for Withdraw to differentiate from Deposit */}
        <div className="bg-gradient-to-r from-amber-600 via-orange-500 to-amber-600 px-5 py-4 text-white flex items-center justify-between relative overflow-hidden">
        ...
        <div className="p-5 sm:p-6">
          ...
          {status === 'idle' && (
            <div className={`p-3.5 rounded-2xl border mb-4 flex items-center justify-between ${
              isDark ? 'bg-[#111827] border-[#1A2332]' : 'bg-amber-50/60 border-amber-100'
            }`}>
              <div>
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-amber-600 to-orange-500 text-white text-[11px] font-black uppercase hover:from-amber-500 hover:to-orange-400 transition-all cursor-pointer shadow-sm"
              >
                Max
              </button>
            </div>
            ...
            <form onSubmit={handleInitiateWithdraw} className="space-y-4">
              ...
              <div>
                <label className={`block text-xs font-extrabold mb-1.5 flex items-center justify-between ${
                  ...
                  <span className="text-[10px] text-amber-500 font-bold uppercase">Auto-Detected</span>
                </label>
                <div className="relative">
                  <select
                    ...
                    className={`w-full pl-3.5 pr-8 py-2.5 rounded-2xl border text-xs sm:text-sm font-bold transition-all outline-none appearance-none cursor-pointer ${
                      isDark
                        ? 'bg-[#111827] border-[#1A2332] text-white focus:border-amber-500'
              ...
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-xs font-black text-amber-500">
                    KSh
                  </span>
                  <input
                    type="number"
                  ...
                  <button
                    ...
                    className={`py-1.5 text-[11px] font-bold rounded-xl border transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed ${
                      parsedAmount === amt
                        ...
                        : isDark
                        ? 'bg-[#111827] border-[#1A2332] text-slate-300 hover:border-amber-500/50'
              ...
              <div className={`p-3 rounded-2xl border text-xs space-y-1.5 ${
                ...
                <div className={`flex justify-between pt-1.5 border-t font-black ${isDark ? 'border-white/5' : 'border-black/5'}`}>
                  ...
                  <span className="text-amber-500">KSh {parsedAmount.toLocaleString()}</span>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-3.5 px-4 rounded-2xl bg-gradient-to-r from-amber-600 via-orange-500 to-amber-600 hover:from-amber-500 hover:to-orange-400 text-white font-black text-xs sm:text-sm tracking-wider uppercase transition-all shadow-lg shadow-amber-600/20 active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
          ...
          {status === 'success' && (
            <div className="py-2 text-center space-y-4">
              <div className="w-14 h-14 rounded-full bg-amber-500/15 border border-amber-500/40 text-amber-500 flex items-center justify-center mx-auto shadow-md">
                <CheckCircle2 className="w-8 h-8" />
              </div>

              <div>
                <h3 className="font-black text-lg text-amber-600 dark:text-amber-400">
                  Payout Dispatched!
                </h3>
              ...
              <div className={`p-3.5 rounded-2xl border text-left text-xs font-mono leading-relaxed ${
                ...
                <div className="flex items-center gap-2 mb-1.5 text-amber-500 font-sans font-bold text-[11px]">
              ...
              <button
                type="button"
                onClick={onClose}
                className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-amber-600 via-orange-500 to-amber-600 hover:from-amber-500 hover:to-orange-400 text-white font-black text-xs uppercase tracking-wider cursor-pointer shadow-lg shadow-amber-600/20"
              >
                Done
              </button>
            </div>
