    ...
    >
      ...
      >
        <defs>
          ...
          {/* Glow filter */}
          <filter id="tq-glow" x="-20%" y="-20%" width="140%" height="140%">
        ...
        <path
          ...
          fill="url(#tq-bolt-grad)"
          filter="url(#tq-glow)"
        />

        {/* Precision Compass Ticks */}
        <line x1="32" y1="8" x2="32" y2="12" stroke="#34D399" strokeWidth="1.5" strokeLinecap="round" />
        <line x1="52" y1="28" x2="48" y2="28" stroke="#34D399" strokeWidth="1.5" strokeLinecap="round" />
        <line x1="12" y1="28" x2="16" y2="28" stroke="#34D399" strokeWidth="1.5" strokeLinecap="round" />

        {/* Core Apex Star Point */}
        <circle cx="32" cy="20" r="1.5" fill="#34D399" />
      </svg>
    </div>
  );
};
...
export const TrivquestLogo: React.FC<TrivquestLogoProps> = ({
  ...
  const sizeMap = {
    xs: { iconSize: 22, textClass: 'text-sm', badgeClass: 'text-[8px] px-1 py-0.2' },
    sm: { iconSize: 28, textClass: 'text-base', badgeClass: 'text-[8.5px] px-1.5 py-0.5' },
    md: { iconSize: 34, textClass: 'text-lg sm:text-xl', badgeClass: 'text-[9px] px-1.5 py-0.5' },
    lg: { iconSize: 48, textClass: 'text-2xl sm:text-3xl', badgeClass: 'text-[10px] px-2 py-0.5' },
    xl: { iconSize: 64, textClass: 'text-3xl sm:text-4xl', badgeClass: 'text-xs px-2.5 py-1' },
  };
    ...
    <div className={`inline-flex items-center gap-2 sm:gap-2.5 select-none ${className}`}>
      ...
      {showWordmark && (
        <div className="flex items-center gap-1.5 leading-none">
          <div className={`font-black tracking-tighter ${conf.textClass} flex items-center`}>
          ...
          <span
            className={`hidden sm:inline-flex rounded font-extrabold uppercase tracking-wider border border-emerald-500/40 bg-emerald-500/10 text-emerald-400 shadow-2xs ${conf.badgeClass}`}
          >
            ARENA
          </span>
        </div>
