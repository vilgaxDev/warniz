...
export const AnswerOption: React.FC<AnswerOptionProps> = ({
  ...
  if (isAnswered) {
    if (isSelected) {
      if (isCorrectOption) {
        ...
        icon = <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 shrink-0 ml-auto text-emerald-400" />;
      } else {
        buttonStyle = isDark
        ...
        icon = <XCircle className="w-4 h-4 sm:w-5 sm:h-5 shrink-0 ml-auto text-rose-400" />;
      }
      ...
      icon = <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-400 shrink-0 ml-auto" />;
    } else {
  ...
  return (
    <button
      ...
      disabled={isAnswered}
      className={`w-full min-h-[56px] sm:min-h-[64px] p-2.5 sm:p-3.5 rounded-xl sm:rounded-2xl border text-left transition-all duration-150 flex items-center gap-2 sm:gap-3 group cursor-pointer disabled:cursor-default select-none shadow-xs active:scale-[0.98] ${buttonStyle}`}
    >
      {/* Option Letter Badge (A, B, C, D) or Check for True/False */}
      <span className={`w-6 h-6 sm:w-7 sm:h-7 rounded-lg border flex items-center justify-center font-mono text-[11px] sm:text-xs font-black shrink-0 transition-colors shadow-2xs ${badgeStyle}`}>
        {isTrueOption ? '✓' : isFalseOption ? '✗' : label}
      </span>

      {/* Option Text */}
      <span className={`flex-1 text-xs sm:text-sm font-semibold leading-tight line-clamp-2 tracking-tight ${optionTextColor}`}>
        {optionText}
      </span>
