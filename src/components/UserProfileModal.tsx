      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-sm font-sans">
        <motion.div
          ...
          className={`w-full max-w-3xl rounded-2xl border flex flex-col max-h-[92vh] overflow-hidden shadow-2xl relative ${
            isDark ? 'bg-[#12100F] border-[#292524] text-[#F5F5F5]' : 'bg-white border-slate-200 text-slate-900'
          }`}
        >
          {/* Header Bar */}
          <div className={`p-4 sm:p-5 border-b flex items-center justify-between shrink-0 ${
            isDark ? 'border-[#292524] bg-[#080403]' : 'border-slate-100 bg-slate-50'
          }`}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-[#F55129]/20 border border-[#F55129]/40 text-2xl flex items-center justify-center shrink-0 shadow-md">
                {userProfile.avatar}
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <h2 className={`font-semibold text-base sm:text-lg leading-tight ${isDark ? 'text-[#F5F5F5]' : 'text-slate-900'}`}>
                    {userProfile.name}
                  </h2>
                  <span className="text-[10px] px-2 py-0.5 rounded-md bg-[#F55129]/15 text-[#F55129] border border-[#F55129]/30 font-semibold uppercase">
                    RANK #{userProfile.rank}
                  </span>
                </div>
                <p className={`text-xs ${isDark ? 'text-[#9CA3AF]' : 'text-slate-500'}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`p-2 rounded-xl border transition-colors cursor-pointer ${
                  isDark ? 'bg-[#181513] border-[#292524] text-[#9CA3AF] hover:text-[#F5F5F5]' : 'bg-slate-100 border-slate-200 text-slate-600 hover:text-slate-900'
                }`}
              >
                <X className="w-4 h-4" />
              </button>
          ...
          <div className={`grid grid-cols-4 p-1.5 border-b text-xs font-semibold shrink-0 ${
            isDark ? 'bg-[#080403] border-[#292524]' : 'bg-slate-100 border-slate-200'
          }`}>
            <button
              onClick={() => setActiveTab('profile')}
              className={`py-2.5 px-2 rounded-xl transition-colors cursor-pointer flex items-center justify-center gap-1.5 ${
                activeTab === 'profile'
                  ? 'bg-[#F55129] text-white shadow-xs'
                  : isDark ? 'text-[#9CA3AF] hover:text-[#F5F5F5]' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
          ...
          <div className="p-4 sm:p-6 overflow-y-auto flex-1">
            ...
            {activeTab === 'profile' && (
              <div className="space-y-6">
                ...
                <div className={`p-4 sm:p-5 rounded-2xl border flex flex-col sm:flex-row items-center justify-between gap-4 ${
                  isDark ? 'bg-[#181513] border-[#292524]' : 'bg-emerald-50 border-emerald-200'
                }`}>
                  <div>
                    ...
                    <p className={`text-xs mt-1 ${isDark ? 'text-[#9CA3AF]' : 'text-slate-600'}`}>
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    <button
                      ...
                      className="flex-1 sm:flex-initial py-2.5 px-4 rounded-xl bg-[#F55129] hover:bg-[#DB3211] text-white font-semibold text-xs flex items-center justify-center gap-1.5 cursor-pointer shadow-md transition-colors"
                    >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  <div className={`p-3.5 rounded-xl border ${isDark ? 'bg-[#181513] border-[#292524]' : 'bg-slate-50 border-slate-200'}`}>
                    ...
                    <div className="text-lg sm:text-xl font-semibold text-[#E28C6D]">
                      {accuracyPercent}%
                    </div>
                  </div>
                  ...
                  <div className={`p-3.5 rounded-xl border ${isDark ? 'bg-[#181513] border-[#292524]' : 'bg-slate-50 border-slate-200'}`}>
                    ...
                    <div className="text-lg sm:text-xl font-semibold text-[#F5F5F5]">
                  ...
                  <div className={`p-3.5 rounded-xl border ${isDark ? 'bg-[#181513] border-[#292524]' : 'bg-slate-50 border-slate-200'}`}>
                    ...
                    <div className="text-lg sm:text-xl font-semibold text-[#F5F5F5] flex items-center gap-1.5">
            ...
            {activeTab === 'wallet' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className={`font-semibold text-xs uppercase tracking-wider ${isDark ? 'text-[#9CA3AF]' : 'text-slate-500'}`}>
                    Transaction History
                  </h3>
                    ...
                    {(['all', 'deposit', 'withdrawal', 'quiz_reward'] as const).map((filter) => (
                      <button
                        ...
                        className={`px-2.5 py-1 rounded-lg border font-semibold capitalize cursor-pointer transition-colors ${
                          txFilter === filter
                            ? 'bg-[#F55129] text-white border-[#F55129]'
                            : isDark ? 'bg-[#181513] border-[#292524] text-[#9CA3AF] hover:text-[#F5F5F5]' : 'bg-slate-100 border-slate-200 text-slate-600'
                        }`}
                      >
                    ...
                    filteredTransactions.map((tx) => (
                      <div
                        ...
                        className={`p-3 rounded-xl border flex items-center justify-between gap-3 ${
                          isDark ? 'bg-[#181513] border-[#292524]' : 'bg-slate-50 border-slate-200'
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-xs sm:text-sm text-[#F5F5F5]">{tx.title}</div>
                            <div className="text-[11px] text-[#9CA3AF]">{tx.timestamp} • Ref: {tx.mpesaRef}</div>
                          </div>
                        </div>
                    ...
                    questionHistory.map((item) => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-[10px] px-2 py-0.5 rounded-md font-semibold uppercase bg-[#F55129]/15 text-[#F55129] border border-[#F55129]/30">
                            {item.category}
                          </span>
                          <span className="text-[11px] text-[#9CA3AF] font-medium">
                            {item.timestamp}
                        ...
                        <p className="text-xs sm:text-sm font-semibold leading-relaxed text-[#F5F5F5]">
                          {item.questionText}
                        </p>

                        <div className="grid grid-cols-2 gap-2 text-xs pt-1 border-t border-[#292524]">
                          <div>
                            <span className="text-[#9CA3AF] text-[10px] uppercase font-semibold block">Your Answer:</span>
                            <span className={`font-semibold ${item.isCorrect ? 'text-[#22C55E]' : 'text-[#EF4444]'}`}>
                              {item.userAnswer}
                            </span>
                          </div>
                          <div>
                            <span className="text-[#9CA3AF] text-[10px] uppercase font-semibold block">Correct Answer:</span>
                ...
                <form onSubmit={handleSaveProfile} className="space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <label className={`block text-xs font-semibold ${isDark ? 'text-[#F5F5F5]' : 'text-slate-700'}`}>
                        Country / Region Flag & Location
                      </label>
                      <span className="text-[10px] text-[#E28C6D] font-semibold flex items-center gap-1">
                    ...
                    <button
                      ...
                      className={`w-full p-2.5 rounded-xl border text-xs font-semibold flex items-center justify-between gap-2.5 cursor-pointer transition-all hover:border-[#F55129] ${
                        isDark
                          ? 'bg-[#181513] border-[#292524] text-[#F5F5F5]'
                    ...
                    >
                      <div className="flex items-center gap-2.5 truncate">
                        ...
                        <span className="font-semibold truncate text-[#F5F5F5]">{selectedCountry.name}</span>
                        <span className="text-[11px] font-mono text-[#E28C6D] shrink-0">({selectedCountry.dialCode})</span>
                      </div>
                    ...
                    <div className="mt-1 flex items-center justify-between text-[10px] px-1 text-[#9CA3AF]">
                      <span>Payout: <strong className="text-[#22C55E] truncate max-w-[200px]">{selectedCountry.paymentMethod}</strong></span>
                      <button
                        type="button"
                        onClick={() => setIsCountryPickerOpen(true)}
                        className="text-[#F55129] font-semibold hover:underline cursor-pointer"
                      >
                  ...
                  <div>
                    <label className={`block text-xs font-semibold mb-1.5 ${isDark ? 'text-[#F5F5F5]' : 'text-slate-700'}`}>
                      Display Name
                    </label>
                    <input
                      type="text"
                      ...
                      className={`w-full px-3.5 py-2.5 rounded-xl border text-xs font-medium focus:outline-hidden focus:ring-2 focus:ring-[#F55129] ${
                        isDark ? 'bg-[#181513] border-[#292524] text-[#F5F5F5]' : 'bg-white border-slate-300 text-slate-900 shadow-2xs'
                      }`}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        type="button"
                        onClick={() => setIsCountryPickerOpen(true)}
                        className={`h-[42px] px-3 rounded-xl border flex items-center gap-1.5 text-xs font-semibold shrink-0 transition-all cursor-pointer hover:border-[#F55129] ${
                          isDark
                            ? 'bg-[#181513] border-[#292524] text-[#F5F5F5] hover:bg-[#1A1715]'
                            : 'bg-slate-100 border-slate-300 text-slate-800 hover:bg-slate-200 shadow-2xs'
                        }`}
                        title="Click to change country flag"
                      >
                        <span className="text-lg leading-none">{selectedCountry.flag}</span>
                        <span className="font-mono text-[#E28C6D]">{selectedCountry.dialCode}</span>
                      ...
                      <input
                        ...
                        className={`flex-1 px-3.5 py-2.5 rounded-xl border text-xs font-medium font-mono focus:outline-hidden focus:ring-2 focus:ring-[#F55129] ${
                          isDark ? 'bg-[#181513] border-[#292524] text-[#F5F5F5]' : 'bg-white border-slate-300 text-slate-900 shadow-2xs'
                        }`}
                      />
                    </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="text-[10px] text-[#9CA3AF]">
                        Flag: <strong className="text-[#F55129]">{selectedCountry.name} ({selectedCountry.flag})</strong>
                      </span>
                    </div>
                      ...
                      {getAvatarOptionsForCountry(selectedCountry.flag).map((av) => (
                        <button
                          ...
                          className={`w-9 h-9 rounded-xl border text-lg flex items-center justify-center shrink-0 cursor-pointer ${
                            editAvatar === av
                              ? 'bg-[#F55129]/20 border-[#F55129] ring-2 ring-[#F55129]'
                              : isDark ? 'bg-[#181513] border-[#292524]' : 'bg-slate-100 border-slate-200'
                          }`}
                        >
                          {av}
                        </button>
            ...
            {activeTab === 'terms' && (
              <div className="space-y-4 text-xs leading-relaxed max-w-xl mx-auto">
                <div className={`flex items-center gap-2 pb-2 border-b ${isDark ? 'border-[#292524]' : 'border-slate-200'}`}>
                  <FileText className="w-4 h-4 text-[#F55129]" />
                  <h3 className={`font-semibold text-sm ${isDark ? 'text-[#F5F5F5]' : 'text-slate-900'}`}>Terms of Service</h3>
                </div>
                <p className={isDark ? 'text-[#9CA3AF]' : 'text-slate-700'}>1. <strong>Fair Play:</strong> Each question is bound to an immutable countdown clock. Results are settled instantly.</p>
                <p className={isDark ? 'text-[#9CA3AF]' : 'text-slate-700'}>2. <strong>Withdrawals:</strong> Immediate payout via supported global payment rails with 0% processing fee.</p>
                <p className={isDark ? 'text-[#9CA3AF]' : 'text-slate-700'}>3. <strong>Wallet Safety:</strong> Funds are safeguarded in an isolated transparent pool.</p>
              </div>
            )}
            ...
            {activeTab === 'privacy' && (
              <div className="space-y-4 text-xs leading-relaxed max-w-xl mx-auto">
                <div className={`flex items-center gap-2 pb-2 border-b ${isDark ? 'border-[#292524]' : 'border-slate-200'}`}>
                  ...
                  <h3 className={`font-semibold text-sm ${isDark ? 'text-[#F5F5F5]' : 'text-slate-900'}`}>Privacy & Security</h3>
                </div>
                <p className={isDark ? 'text-[#9CA3AF]' : 'text-slate-700'}>1. <strong>Data Encryption:</strong> All transactions and session keys are secured with 256-bit AES encryption.</p>
                <p className={isDark ? 'text-[#9CA3AF]' : 'text-slate-700'}>2. <strong>Payment Privacy:</strong> We never store private banking PINs. All authentication occurs via secure external gateway prompts.</p>
              </div>
            )}
          </div>
          ...
          {isDepositOpen && (
            <div className="absolute inset-0 z-30 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
              <div className={`w-full max-w-md p-5 sm:p-6 rounded-2xl border ${
                isDark ? 'bg-[#12100F] border-[#292524] text-[#F5F5F5]' : 'bg-white border-slate-300 text-slate-900'
              }`}>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-semibold text-base flex items-center gap-2 text-[#F5F5F5]">
                    <Plus className="w-4 h-4 text-[#F55129]" />
                    <span>Instant Deposit ({selectedCountry.currency})</span>
                  </h3>
                  <button onClick={() => setIsDepositOpen(false)} className="text-[#9CA3AF] hover:text-[#F5F5F5] cursor-pointer">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                <form onSubmit={handleExecuteDeposit} className="space-y-4">
                  <div>
                    <label className="block text-xs font-semibold mb-1.5 text-[#F5F5F5]">Deposit Amount ({selectedCountry.currency})</label>
                      ...
                      {['100', '200', '500', '1000'].map((val) => (
                        <button
                          ...
                          className={`py-1.5 text-xs font-semibold rounded-lg border cursor-pointer ${
                            depositAmount === val
                              ? 'bg-[#F55129] text-white border-[#F55129]'
                              : isDark ? 'bg-[#181513] border-[#292524] text-[#9CA3AF]' : 'bg-slate-100 border-slate-200'
                          }`}
                        >
                          +{val}
                        </button>
                    ...
                    <input
                      ...
                      className={`w-full px-3.5 py-2.5 rounded-xl border text-sm font-semibold ${
                        isDark ? 'bg-[#181513] border-[#292524] text-[#F5F5F5]' : 'bg-slate-50 border-slate-300 text-slate-900'
                      }`}
                    />
                  </div>
                  ...
                  <button
                    type="submit"
                    className="w-full py-3 rounded-xl bg-[#F55129] hover:bg-[#DB3211] text-white font-semibold text-xs flex items-center justify-center gap-2 shadow-md cursor-pointer"
                  >
            ...
            <div className="absolute inset-0 z-30 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
              <div className={`w-full max-w-md p-5 sm:p-6 rounded-2xl border ${
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={() => setIsWithdrawOpen(false)} className="text-[#9CA3AF] hover:text-[#F5F5F5] cursor-pointer">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                <form onSubmit={handleExecuteWithdraw} className="space-y-4">
                  <div>
                    <label className="block text-xs font-semibold mb-1.5 text-[#F5F5F5]">
